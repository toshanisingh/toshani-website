...
check("images survive sanitization (the headline feature) with size", () => {
  const html = renderContent(
    docWith([{ type: "image", attrs: { src: "https://cdn.example.com/a.png", alt: "pic", size: "medium" } }]),
  );
...
check("font-family, color, text-align and underline survive", () => {
  const html = renderContent(
    docWith([
      {
        type: "paragraph",
        ...
        content: [
          { type: "text", marks: [{ type: "textStyle", attrs: { fontFamily: "Lora, serif" } }], text: "fancy" },
          { type: "text", marks: [{ type: "textStyle", attrs: { color: "#ff0000" } }], text: "red" },
          { type: "text", marks: [{ type: "underline" }], text: "under" },
        ],
      },
      { type: "heading", attrs: { level: 5, textAlign: "right" }, content: [{ type: "text", text: "H5" }] },
    ]),
  );
...
check("malicious style values are stripped", () => {
  const html = renderContent(
    docWith([
      {
        type: "paragraph",
        content: [
          { type: "text", marks: [{ type: "textStyle", attrs: { fontFamily: "x; background:url(javascript:alert(1))" } }], text: "evil" },
        ],
      },
    ]),
  );
