...
const { encodeToken, decodeToken, versionFromBasis } = await import("./token-codec.ts");
...
check("version changes with password state (the single-use mechanism)", () => {
  // verifyResetToken rejects a token when payload.v !== version of the current
  // passwordHash. So the property that matters is: the version actually changes
  // when the password state changes.
  const setup = versionFromBasis("setup"); // no account yet
  const afterFirst = versionFromBasis("$2b$12$hashAfterFirstPasswordSet");
  const afterReset = versionFromBasis("$2b$12$hashAfterSecondReset");

  // Setting the first password changes the version -> setup link is now stale.
  assert.notEqual(setup, afterFirst);
  // A later reset changes it again -> the previous reset link is now stale.
  assert.notEqual(afterFirst, afterReset);
  // Deterministic for a given basis (so a still-valid token keeps verifying).
  assert.equal(afterFirst, versionFromBasis("$2b$12$hashAfterFirstPasswordSet"));
});
